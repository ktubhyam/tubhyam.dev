...
import LatentScatterViz from '@components/islands/LatentScatterViz';
import GradientFlowViz from '@components/islands/GradientFlowViz';
import DisentanglementTracker from '@components/islands/DisentanglementTracker';
...
<LatentScatterViz client:visible />
...
<EquationReveal
  ...
  equations={[
    {
      label: "VIB Objective",
      annotation: "Classification log-likelihood minus information cost — the standard formulation (Alemi et al. 2017). Here −log p(y|z) is cross-entropy, not reconstruction.",
      segments: [
...
In the standard VIB formulation (Alemi et al. 2017), the first term is a classification log-likelihood — $-\log p(y \mid z)$ is cross-entropy measuring how well $z$ predicts the target $Y$ (e.g. molecule identity). Spektron uses a VAE-VIB hybrid where that classification term is replaced with a <span class="highlight">masked reconstruction loss</span> $\mathcal{L}_\text{MAE}$: instead of predicting molecule labels, the bottleneck must preserve enough information to reconstruct masked spectral patches. The second term is a KL divergence that regularizes the posterior toward a standard Gaussian prior — the same as a VAE, but the motivation differs. We're not trying to generate spectra; we're trying to <span class="highlight">forget instrument-specific information</span> while keeping chemistry.
...
<GradientFlowViz client:visible />
...
Posterior collapse is worth taking seriously at 128 dimensions. With $\beta_\text{start} = 0.1$ and 128 latent dims, the KL penalty is strong enough to push many dimensions to the prior ($\mu \approx 0$, $\sigma \approx 1$, contributing zero information). The per-dimension KL diagnostic catches this early: if more than 30% of $z_\text{chem}$ dimensions have $D_\text{KL}(\mathcal{N}(\mu_i, \sigma_i^2) \| \mathcal{N}(0,1)) < 0.01$ at step 5K, you're collapsing. An alternative that avoids this entirely is <span class="highlight">cyclical annealing</span> (Fu et al. 2019): instead of monotonically decreasing $\beta$, it cycles it — rise, high plateau, fall — multiple times. Each cycle gives the model a chance to activate new latent dimensions that collapsed in the previous cycle. For 128-dim bottlenecks on large datasets, cyclical annealing tends to activate 20-30% more latent dimensions than monotone annealing.

The cosine schedule resolves the fixed-$\beta$ dilemma: explore first (high $\beta$), then exploit (low $\beta$). The 60% annealing window was determined empirically — shorter windows don't allow enough exploration, while longer windows delay the tightening phase and reduce final discriminability.
...
<DisentanglementTracker client:visible />
...
A word of caution: chance-level domain accuracy is a <span class="highlight">necessary but not sufficient</span> condition for disentanglement. A model that maps all inputs to the same point achieves 50% domain accuracy trivially — but it also encodes zero chemistry. Locatello et al. (2019) proved that fully unsupervised disentanglement is impossible without inductive biases; gradient reversal provides exactly that bias (instrument labels during training), so this is weakly-supervised disentanglement, not unsupervised. Always check molecule accuracy on $z_{\text{chem}}$ alongside domain accuracy. If molecule accuracy is below 80% at chance-level domain accuracy, the model has collapsed — not disentangled.
...
$$\alpha(p) = \frac{2}{1 + \exp(-\gamma \cdot p)} - 1, \quad p = \frac{\text{step}}{\text{total steps}}, \quad \gamma = 10$$

This sigmoid schedule rises slowly from 0, accelerates through midtraining, and saturates at 1.0 — front-loading the easy learning and gradually introducing adversarial pressure as the encoder matures. We use $\gamma = 10$ as in the original paper, which produces near-linear warmup over the first 20% of training. Without warmup (any schedule), training loss oscillates wildly and the encoder learns degenerate constant-output features.
