...
title: "ReactorTwin"
date: 2026-02-28
description: "Physics-constrained neural ODE surrogate for chemical reactor digital twins. v1.1.1, production stable — 1500x faster than first-principles simulation."
tags: ["pytorch", "neural-odes", "chemical-engineering", "digital-twin"]
status: "active"
github: "https://github.com/ktubhyam/reactor-twin"
featured: true
---

ReactorTwin creates fast, accurate surrogate models of chemical reactors that run 1500x faster than traditional first-principles simulation while preserving physical constraints. v1.1.1 is production-stable: deployable via Docker, Kubernetes, or CLI with a full digital twin stack (EKF state estimation, fault detection, MPC control, online adaptation, meta-learning).
...
A real-time dashboard built with Streamlit provides:
...
- **Core:** PyTorch + torchdiffeq — Neural ODE integration with continuous adjoint backprop
- **Dashboard:** Streamlit — 10-page interactive dashboard (phase portraits, bifurcation, RTD, sensitivity, fault monitoring, latent space explorer)
- **Serving:** FastAPI + Docker — REST API v1/v2 with JWT auth, rate limiting, WebSocket streaming
- **Deploy:** Kubernetes Helm chart with HPA + GPU Dockerfile; CLI (`reactor-twin train | serve | export | dashboard`)
- **Export:** ONNX Runtime — NeuralODE/Latent/Augmented → ONNX for edge inference
...
ReactorTwin is the codebase for **"Choosing Physics Constraints for Neural ODE Reactor Surrogates"** (ML4PS @ NeurIPS 2026). Key findings: hard post-solve enforcement achieves 0.00% violations everywhere and 99% NMSE reduction on exothermic CSTR; soft ℓ₂ penalties produce seed-dependent, λ-insensitive violations (0–41%) within tested budgets. The right inductive bias depends on state structure.
