...
## The Architecture: Spekron

The theory says what is achievable. The model is designed to get there. **Spekron** is a CNN-Transformer encoder with a <span class="highlight">Variational Information Bottleneck</span> (VIB) that splits the latent space into chemistry and instrument:
...
<TerminalBlock
  ...
  lines={[
    { spans: [{ text: "$ python -c \"from spekron import SpectralFM; print(SpectralFM())\"", color: "muted" }] },
    { spans: [{ text: "" }], delay: 200 },
    { spans: [{ text: "SpectralFM(", color: "white" }] },
    { spans: [
...
<TerminalBlock
  ...
  lines={[
    { spans: [{ text: "$ spekron status", color: "muted" }] },
    { spans: [{ text: "" }], delay: 200 },
...
- **Research paper:** [Hybrid SSA Spectroscopy](/research/hybrid-ssa-spectroscopy) — the Spekron architecture paper
...
- **Project:** [Spekron](/projects/spekron) — the foundation model implementation
- **Preprocessing:** [SpectraKit](/projects/spectrakit) — the spectral preprocessing library used in the pipeline
